...
def get_access_token():
    ...
    consumer_key = getattr(settings, 'CONSUMER_KEY', 'H3jdfPuU3srfX2uo7LFQ1w')
    consumer_secret = getattr(settings, 'CONSUMER_SECRET', 'Fe0iHcfi8nubMBzjbcUuf6zRW8Nn9VgMJkiHcCdKwSw')
    ...
    oauth_client.set_signature_method(signature_method_hmac_sha1)
    print 'Requesting temp token from Twitter'

    resp, content = oauth_client.request(REQUEST_TOKEN_URL, 'POST', force_auth_header=True, 
                                         parameters={'oauth_callback': 'http://www.stanford.edu'})
