...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('twitter_cachedstatus', (
            ('status_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['twitter.Status'], unique=True, primary_key=True)),
            ('stored', self.gf('django.db.models.fields.BooleanField')(default=False)),
        ))
        db.send_create_signal('twitter', ['CachedStatus'])

        # Adding unique constraint on 'Rating', fields ['status', 'user']
        db.create_unique('twitter_rating', ['status_id', 'user_id'])

        # Adding M2M table for field cached_statuses on 'TwitterUserProfile'
        db.create_table('twitter_twitteruserprofile_cached_statuses', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('twitteruserprofile', models.ForeignKey(orm['twitter.twitteruserprofile'], null=False)),
            ('cachedstatus', models.ForeignKey(orm['twitter.cachedstatus'], null=False))
        ))
        db.create_unique('twitter_twitteruserprofile_cached_statuses', ['twitteruserprofile_id', 'cachedstatus_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Rating', fields ['status', 'user']
        db.delete_unique('twitter_rating', ['status_id', 'user_id'])

        # Deleting model 'CachedStatus'
        db.delete_table('twitter_cachedstatus')

        # Removing M2M table for field cached_statuses on 'TwitterUserProfile'
        db.delete_table('twitter_twitteruserprofile_cached_statuses')
    ...
    models = {
        ...
        'twitter.cachedstatus': {
            'Meta': {'ordering': "['-id']", 'object_name': 'CachedStatus', '_ormbases': ['twitter.Status']},
            'status_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['twitter.Status']", 'unique': 'True', 'primary_key': 'True'}),
        ...
        'twitter.twitteruserprofile': {
            ...
            'cached_statuses': ('django.db.models.fields.related.ManyToManyField', [], {'blank': 'True', 'related_name': "'cached_statuses'", 'null': 'True', 'symmetrical': 'False', 'to': "orm['twitter.CachedStatus']"}),
