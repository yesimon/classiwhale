...
raters_screennames = ['classiwhale{0}'.format(i) for i in raters]
...
def FullCreateStatus(status):
    ...
    s.user_profile_id = status.user.id
...
def CreateTrainingSet(file):
    train_set = pickle.loads(file.read().replace('\r\n', '\n'))
...
def main():
    """
    Evaluate command line arguemnts - works with 2.7 and 2.6 
    through introspection of code
    """
    def Parser27():
        import argparse
    ...
    def Parser26():
        import optparse
        parser = optparse.OptionParser(usage='Create classiwhale training set for human ratings.')
        parser.add_option("--get", action='store_true',
            default=False, dest="GET")
        parser.add_option("--create", action='store_true',
            default=False, dest="CREATE")
        parser.add_option("--dump", action='store_true',
            default=False, dest="DUMP")
        (opts, args) = parser.parse_args() 
        if opts.GET: opts.GET = open('train_set.pkl', 'w')
        if opts.CREATE: opts.CREATE = open('train_set.pkl', 'r')
        if opts.DUMP: opts.DUMP = open('train_set_dump.pkl', 'w')
        return opts
